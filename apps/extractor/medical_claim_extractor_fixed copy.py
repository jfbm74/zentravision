...
class MedicalClaimExtractor:
    ...
    def _extract_procedure_from_line(self, line: str, all_lines: List[str], line_idx: int, table_end: int) -> Optional[Dict[str, Any]]:
        ...
        # Patrón 2: Solo código en la línea
        ...
        # Patrón 3: Procedimiento sin código (como VENDA ELASTICA)
    ...
    def _extract_observation_from_lines(self, lines: List[str], start_idx: int, end_idx: int) -> str:
        ...
        obs_match = re.search(r'(\d{4})\s*>>\s*(.+?)$', current_line)
        if obs_match:
        ...
        for i in range(start_idx + 1, min(start_idx + 5, end_idx)):
            ...
            if re.match(r'^\d{4}\s*>>', line):
                ...
                # Continuación de observación
                continuation = line[2:].strip()
                if observation:
                    observation += " " + continuation
                else:
                    observation = continuation
            elif re.match(r'^\d{5}', line) or 'Total' in line:
                # Nueva línea de procedimiento o fin de tabla
                break
    ...
    def _build_enhanced_openai_prompt(self, text: str) -> str:
        """Construye prompt mejorado para OpenAI que captura TODOS los procedimientos"""
        ...
        5. Busca ESPECÍFICAMENTE en la tabla que tiene columnas como:
           Código | Descripción | Cant | Valor total | Valor pagado | Valor objetado
    ...
    def _count_duplicated_codes_in_pdf(self, text: str) -> Dict[str, int]:
        """Método de diagnóstico para contar códigos duplicados en el PDF"""
        code_counts = {}
        
        # Buscar todos los códigos en el texto
        lines = text.split('\n')
        for line in lines:
            # Buscar códigos de 5 dígitos al inicio de línea
            match = re.match(r'^(\d{5})\s+', line.strip())
            if match:
                code = match.group(1)
                code_counts[code] = code_counts.get(code, 0) + 1
        
        # Log de códigos duplicados
        duplicated = {code: count for code, count in code_counts.items() if count > 1}
        if duplicated:
            logger.info("Códigos duplicados encontrados en PDF:")
            for code, count in duplicated.items():
                logger.info(f"  Código {code}: {count} veces")
    ...
    def _are_descriptions_similar(self, desc1: str, desc2: str) -> bool:
        ...
        Para evitar duplicados de procedimientos con descripciones ligeramente diferentes
        """
        ...
        # Si una está contenida en la otra (para casos donde una es más detallada)
        if desc1_norm in desc2_norm or desc2_norm in desc1_norm:
            return True
        
        # Comparar las primeras palabras significativas
        words1 = desc1_norm.split()[:3]  # Primeras 3 palabras
        words2 = desc2_norm.split()[:3]
        
        # Si al menos 2 de las primeras 3 palabras coinciden
    ...
    def debug_table_extraction(self, text: str):
        """Debug para análisis de extracción de tabla"""
        lines = text.split('\n')
        ...
        # Buscar la tabla
        for i, line in enumerate(lines):
            if re.search(r'Código\s+Descripción\s+Cant', line, re.IGNORECASE):
                print(f"\nEncabezado de tabla encontrado en línea {i}: {line}")
                
                # Mostrar las siguientes 20 líneas
                print("\n--- Contenido de la tabla ---")
                for j in range(i, min(i + 20, len(lines))):
                    print(f"Línea {j}: {lines[j]}")
                break
